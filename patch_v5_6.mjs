...
// Replace v5_5 with v5_6 in strings
content = content.replace(/v5_5/g, 'v5_6');
content = content.replace(/V5\.5/g, 'V5.6');
...
let endSqlBlock = content.indexOf(`fs.writeFileSync(path.join(dbDir, '04_seed_kassia_skeleton_preview_v5_6.sql'), sqlOut);`);
...
content = content.substring(0, splitStart) + newLogic + content.substring(endSqlBlock + `fs.writeFileSync(path.join(dbDir, '04_seed_kassia_skeleton_preview_v5_6.sql'), sqlOut);`.length);

fs.writeFileSync('scripts/generate_kassia_skeleton_seed_v5_6.mjs', content);
