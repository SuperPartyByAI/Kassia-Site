...
function getLabelOwner(slot) {
  if (slot === 'hero_image') return 'Imagine principală';
  if (slot.startsWith('gallery_')) return `Imagine galerie ${slot.split('_')[1]}`;
  return 'Imagine';
}
...
const preflightData = {
  ...
  live_db_preflight_executed: false,
  ...
  page_type_safe_for_all_values: schemaContent.includes('page_type VARCHAR'),
  duplicate_existing_rows: "NOT_CHECKED_LIVE"
};

fs.writeFileSync(path.join(docsDir, 'kassia_seed_preflight_v5_3.json'), JSON.stringify(preflightData, null, 2));

let preflightMd = `# Kassia Seed Preflight V5.3\n\n`;
...
preflightMd += `- **kassia_pages.page_type safe for all values**: ${preflightData.page_type_safe_for_all_values ? 'DA' : 'NU'}\n`;
preflightMd += `- **Existing duplicates**: ${preflightData.duplicate_existing_rows}\n`;

fs.writeFileSync(path.join(docsDir, 'kassia_seed_preflight_v5_3.md'), preflightMd);
...
const previewData = {
  summary: {
    ...
    faq_placeholders_publish_blocking: true
  },
  pages
};

fs.writeFileSync(path.join(docsDir, 'kassia_seed_preview_from_skeleton_v5_3.json'), JSON.stringify(previewData, null, 2));

let mdContent = `# Kassia Seed Preview V5.3 (169 Pages)\n\n`;
...
mdContent += `- FAQ placeholders marked publish-blocking: ${previewData.summary.faq_placeholders_publish_blocking ? 'DA' : 'NU'}\n\n`;

fs.writeFileSync(path.join(docsDir, 'kassia_seed_preview_from_skeleton_v5_3.md'), mdContent);

// Generate SQL 04_seed_kassia_skeleton_preview_v5_3.sql
let sqlOut = `-- SEED SKELETON PREVIEW PENTRU KASSIA V5.3 (169 PAGES)\n`;
...
sqlOut += `    IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'unique_internal_links' OR conname = 'kassia_internal_links_source_page_id_target_page_id_anchor__key') THEN\n`;
...
fs.writeFileSync(path.join(dbDir, '04_seed_kassia_skeleton_preview_v5_3.sql'), sqlOut);

console.log(JSON.stringify(previewData.summary, null, 2));
fs.writeFileSync(path.join(docsDir, 'kassia_internal_link_graph_v5_3.json'), JSON.stringify(links, null, 2));
