...
function addLink(source, target, anchor, reason) {
  ...
  if (links.find(l => l.source === source && l.target === target)) {
    duplicateLinksCount++;
    console.log(`Duplicate link skipped: ${source} -> ${target}`);
    return;
  }
...
fs.writeFileSync(path.join(docsDir, 'kassia_seed_preview_from_skeleton_v2.json'), JSON.stringify(previewData, null, 2));
...
fs.writeFileSync(path.join(docsDir, 'kassia_anti_canibalization_map_v2.json'), JSON.stringify(antiCannibalizationMap, null, 2));

fs.writeFileSync(path.join(docsDir, 'kassia_internal_link_graph_v2.json'), JSON.stringify(links, null, 2));


let mdContent = `# Kassia Seed Preview V2 (169 Pages)\n\n`;
...
fs.writeFileSync(path.join(docsDir, 'kassia_seed_preview_from_skeleton_v2.md'), mdContent);

// Generate SQL 04_seed_kassia_skeleton_preview_v2.sql
let sqlOut = `-- SEED SKELETON PREVIEW PENTRU KASSIA V2 (169 PAGES)\n`;
...
fs.writeFileSync(path.join(dbDir, '04_seed_kassia_skeleton_preview_v2.sql'), sqlOut);
