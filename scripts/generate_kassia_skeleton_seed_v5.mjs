...
function cleanBlogTitle(slugStr) {
  const clean = slugStr.replace(/-/g, ' ');
  const capitalized = clean.charAt(0).toUpperCase() + clean.slice(1);
  const isQuestion = /^(cat |ce |cum |unde |care |cand |se pot |cate |cu cat |cu ce )/i.test(clean);
...
fs.writeFileSync(path.join(docsDir, 'kassia_seed_preflight_v5.json'), JSON.stringify(preflightData, null, 2));

let preflightMd = `# Kassia Seed Preflight V5\n\n`;
...
fs.writeFileSync(path.join(docsDir, 'kassia_seed_preflight_v5.md'), preflightMd);
...
fs.writeFileSync(path.join(docsDir, 'kassia_seed_preview_from_skeleton_v5.json'), JSON.stringify(previewData, null, 2));

let mdContent = `# Kassia Seed Preview V5 (169 Pages)\n\n`;
...
fs.writeFileSync(path.join(docsDir, 'kassia_seed_preview_from_skeleton_v5.md'), mdContent);

// Generate SQL 04_seed_kassia_skeleton_preview_v5.sql
let sqlOut = `-- SEED SKELETON PREVIEW PENTRU KASSIA V5 (169 PAGES)\n`;
...
pages.forEach(p => {
  ...
  if (p.page_type === 'guide') title = cleanBlogTitle(slug);
...
fs.writeFileSync(path.join(dbDir, '04_seed_kassia_skeleton_preview_v5.sql'), sqlOut);

console.log(JSON.stringify(previewData.summary, null, 2));
fs.writeFileSync(path.join(docsDir, 'kassia_internal_link_graph_v5.json'), JSON.stringify(links, null, 2));
