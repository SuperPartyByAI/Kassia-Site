  ...
  tables.forEach(t => {
    if (openapi.definitions[t]) {
      ...
      Object.keys(props).forEach(c => {
        report[t].columns[c] = {
          type: props[c].type,
          format: props[c].format,
          default: props[c].default
        };
      });
    }
  });
  
  fs.writeFileSync('docs/kassia_live_schema_exact_v5_5.json', JSON.stringify(report, null, 2));
  console.log("Schema types dump saved to docs/kassia_live_schema_exact_v5_5.json");
}
