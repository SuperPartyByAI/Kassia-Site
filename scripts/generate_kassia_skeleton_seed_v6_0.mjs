...
function addLink(source, target, anchor, reason) {
  ...
  if (!rawPages.find(p => p.path === target)) return;
  if (links.find(l => l.source === source && l.target === target)) return;
...
rawPages.forEach(p => {
  ...
  if (p.type === 'homepage') {
    ...
    if (p.path.includes('botez') || p.path.includes('nunta') || p.path.includes('aniversare')) {
        addLink(p.path, '/buchete-baloane-bucuresti/', 'buchete baloane', 'relevant_service');
    }
    ...
    if (parent_path && parent_path !== pilonPath) {
        addLink(p.path, parent_path, 'înapoi la eveniment', 'parent_link');
        // V6.0 Explicit Link from Parent Event to the Satellite
        let prettyAnchor = p.path.replace(/\//g,'').replace(/-/g,' ');
        if (p.path.includes('panou')) prettyAnchor = 'panou foto';
        else if (p.path.includes('arcada')) prettyAnchor = 'arcadă baloane';
        else if (p.path.includes('heliu')) prettyAnchor = 'baloane heliu';
        else if (p.path.includes('buchete')) prettyAnchor = 'buchete baloane';
        
        addLink(parent_path, p.path, prettyAnchor, 'satellite_offer');
    }
    ...
    if (p.path.includes('baloane-heliu')) addLink(p.path, '/baloane-heliu-bucuresti/', 'baloane heliu', 'relevant_service');
    if (p.path.includes('buchete-baloane')) addLink(p.path, '/buchete-baloane-bucuresti/', 'buchete baloane', 'relevant_service');
    if (p.path.includes('roz-auriu') || p.path.includes('alb-auriu')) {
        addLink(p.path, '/galerie/', 'galerie', 'inspiration');
        addLink(p.path, pilonPath, 'decorațiuni baloane București', 'relevant_service');
        // Adaugă link de intrare din pagina principală de galerie spre aceste culori specifice
        addLink('/galerie/', p.path, p.path.replace(/\//g,'').replace(/-/g,' '), 'color_variation');
    }
...
// Explicit blog links requested by user
addLink('/blog/idei-decor-baloane-majorat/', '/panou-foto-baloane-majorat-bucuresti/', 'panou foto majorat', 'inspiration_satellite');
addLink('/blog/idei-decor-baloane-aniversare-adulti/', '/baloane-heliu-aniversare-bucuresti/', 'baloane heliu aniversare', 'inspiration_satellite');
addLink('/blog/idei-decor-baloane-aniversare-adulti/', '/buchete-baloane-aniversare-bucuresti/', 'buchete baloane aniversare', 'inspiration_satellite');
addLink('/blog/baloane-heliu-pentru-majorat-idei/', '/baloane-heliu-majorat-bucuresti/', 'baloane heliu majorat', 'inspiration_satellite');
...
const previewData = {
  summary: {
    ...
    total_planned_links: links.length,
    safe_for_manual_sql_review: true,
    safe_for_seed_run: false
  },
  pages
};

fs.writeFileSync(path.join(docsDir, 'kassia_seed_preview_from_skeleton_v6_0.json'), JSON.stringify(previewData, null, 2));

// Generate SQL 04_seed_kassia_skeleton_preview_v6_0.sql
let sqlOut = `-- SEED SKELETON PREVIEW PENTRU KASSIA V6.0 (169 PAGES)\n`;
sqlOut += `-- GENERATED VIA SCRIPT. FARA CONSTRANGERI DE CONFLICT. SIGURANTA MAXIMA.\n\n`;
sqlOut += `BEGIN;\n\n`;
...
pages.forEach(p => {
  ...
  // Double-ensure no trailing dashes due to slashes logic replacement
  slug = slug.replace(/-+$/, '');
...
fs.writeFileSync(path.join(dbDir, '04_seed_kassia_skeleton_preview_v6_0.sql'), sqlOut);

const uniqueTargets = [...new Set(links.map(l => l.target))];
const uniqueSources = [...new Set(links.map(l => l.source))];
const missingIncoming = pages.filter(p => !uniqueTargets.includes(p.path)).map(p => p.path);
const commercialSatellites = satellitePages;
const orphanedCommercialSatellites = commercialSatellites.filter(s => !uniqueTargets.includes(s));

console.log(JSON.stringify({
  ...previewData.summary,
  missingIncomingCount: missingIncoming.length,
  missingIncomingSample: missingIncoming.slice(0, 5),
  orphanedCommercialSatellites
}, null, 2));

// --- ADDED FOR API DEPLOYMENT ---
const allPages = [
    ...supportPages.map(s => ({ path: s, type: s === '/' ? 'homepage' : 'support' })),
    ...servicePages.map(s => ({ path: s, type: s === '/decoratiuni-baloane-bucuresti/' ? 'service_pillar' : 'service' })),
    ...eventPages.map(s => ({ path: s, type: 'event' })),
...
const outputData = {
    pages, // the processed pages array from inside run()
    links // the processed links array from inside run()
};

fs.writeFileSync('db/seed.json', JSON.stringify(outputData, null, 2));
console.log("JSON seed generated at db/seed.json");
