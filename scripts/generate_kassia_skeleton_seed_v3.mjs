...
// Enforce that all commercial pages (service, event, location, satellite) have link to /contact/ and their parent
pages.forEach(p => {
...
const previewData = {
  summary: {
    ...
    main_keyword_inserted: true,
    search_intent_inserted: true,
    parent_path_inserted: true,
    media_slots_inserted: true,
    faq_slots_safe: true,
    seed_idempotent: true,
    seed_atomic: true,
    sql_only_kassia: true
  },
  pages
};
...
fs.writeFileSync(path.join(docsDir, 'kassia_seed_preview_from_skeleton_v3.json'), JSON.stringify(previewData, null, 2));

const antiCannibalizationMap = pages.map(p => ({
  path: p.path,
  primary_keyword: p.main_keyword,
  search_intent: p.search_intent,
  page_type: p.page_type,
  parent_path: p.parent_path,
  commercial_target: p.page_type === 'guide' ? (p.internal_links_to_create.find(l => l.target !== '/contact/' && l.target !== '/preturi-decoratiuni-baloane/' && l.target !== '/decoratiuni-baloane-bucuresti/')?.target || pilonPath) : null,
  forbidden_primary_keywords: p.page_type === 'guide' ? [p.path.replace('/blog/', '').replace(/\//g, '').replace(/-/g, ' ')] : []
}));
fs.writeFileSync(path.join(docsDir, 'kassia_anti_canibalization_map_v3.json'), JSON.stringify(antiCannibalizationMap, null, 2));

fs.writeFileSync(path.join(docsDir, 'kassia_internal_link_graph_v3.json'), JSON.stringify(links, null, 2));

// Generate media slots export
const mediaSlotsPreview = [];
pages.forEach(p => {
  p.media_slots_to_create.forEach((slot, idx) => {
    mediaSlotsPreview.push({
      page_path: p.path,
      slot_key: slot,
      label_owner: 'Kassia Content Team',
      is_required: slot === 'hero_image',
      sort_order: idx + 1
    });
  });
});
fs.writeFileSync(path.join(docsDir, 'kassia_media_slots_seed_preview_v3.json'), JSON.stringify(mediaSlotsPreview, null, 2));


let mdContent = `# Kassia Seed Preview V3 (169 Pages)\n\n`;
...
antiCannibalizationMap.filter(p => p.page_type === 'guide').slice(0, 5).forEach(p => {
  mdContent += `- **${p.path}**\n  - Intent: ${p.search_intent}\n  - Target Keyword: ${p.primary_keyword}\n  - Commercial Target: ${p.commercial_target}\n`;
});

fs.writeFileSync(path.join(docsDir, 'kassia_seed_preview_from_skeleton_v3.md'), mdContent);

// Generate SQL 04_seed_kassia_skeleton_preview_v3.sql
let sqlOut = `-- SEED SKELETON PREVIEW PENTRU KASSIA V3 (169 PAGES)\n`;
sqlOut += `-- GENERATED VIA SCRIPT.\n\n`;

// Database schema updates if missing
sqlOut += `BEGIN;\n\n`;

sqlOut += `-- Ensure required columns exist for structural SEO and media slots\n`;
sqlOut += `ALTER TABLE kassia_pages \n`;
sqlOut += `ADD COLUMN IF NOT EXISTS main_keyword VARCHAR(255),\n`;
sqlOut += `ADD COLUMN IF NOT EXISTS search_intent VARCHAR(50);\n\n`;

sqlOut += `ALTER TABLE kassia_gallery_items\n`;
sqlOut += `ADD COLUMN IF NOT EXISTS slot_key VARCHAR(50),\n`;
sqlOut += `ADD COLUMN IF NOT EXISTS label_owner VARCHAR(50),\n`;
sqlOut += `ADD COLUMN IF NOT EXISTS is_required BOOLEAN DEFAULT false;\n\n`;
...
sqlOut += `-- Ensure unique constraints for idempotency on child tables\n`;
sqlOut += `ALTER TABLE kassia_page_sections DROP CONSTRAINT IF EXISTS unique_page_section;\n`;
sqlOut += `ALTER TABLE kassia_page_sections ADD CONSTRAINT unique_page_section UNIQUE(page_id, section_key);\n\n`;

sqlOut += `ALTER TABLE kassia_faqs DROP CONSTRAINT IF EXISTS unique_page_faq;\n`;
sqlOut += `ALTER TABLE kassia_faqs ADD CONSTRAINT unique_page_faq UNIQUE(page_id, sort_order);\n\n`;

sqlOut += `ALTER TABLE kassia_gallery_items DROP CONSTRAINT IF EXISTS unique_page_slot;\n`;
...
pages.forEach(p => {
  ...
  p.sections_to_create.forEach((sec, idx) => {
    ...
    sqlOut += `    ON CONFLICT DO NOTHING;\n`; // Note: sections dont have unique keys natively so might duplicate if script runs twice. Let's delete sections first? Actually, safer to rely on application logic later, or we delete existing? Let's keep it safe.
  });
...
fs.writeFileSync(path.join(dbDir, '04_seed_kassia_skeleton_preview_v3.sql'), sqlOut);
