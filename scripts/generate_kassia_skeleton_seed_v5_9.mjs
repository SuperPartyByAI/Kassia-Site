...
fs.writeFileSync(path.join(docsDir, 'kassia_seed_preview_from_skeleton_v5_9.json'), JSON.stringify(previewData, null, 2));

// Generate SQL 04_seed_kassia_skeleton_preview_v5_9.sql
let sqlOut = `-- SEED SKELETON PREVIEW PENTRU KASSIA V5.9 (169 PAGES)\n`;
...
fs.writeFileSync(path.join(dbDir, '04_seed_kassia_skeleton_preview_v5_9.sql'), sqlOut);

console.log(JSON.stringify({
  ...previewData.summary,
  event_service_links: links.filter(l => pages.find(p => p.path === l.source)?.type === 'event' && ['/arcada-baloane-bucuresti/', '/baloane-heliu-bucuresti/', '/panou-foto-baloane-bucuresti/'].includes(l.target)).length > 0,
  satellite_service_links: links.filter(l => pages.find(p => p.path === l.source)?.type === 'satellite' && ['/arcada-baloane-bucuresti/', '/baloane-heliu-bucuresti/', '/panou-foto-baloane-bucuresti/', '/buchete-baloane-bucuresti/'].includes(l.target)).length > 0
}, null, 2));
