...
async function run() {
  ...
  // Task 1: Fix FAQ 3
  const { data: faqsToUpdate } = await supabase.from('kassia_faqs').select('*').eq('page_id', page.id).eq('order_index', 3);
  let dbModified = false;
  if (faqsToUpdate && faqsToUpdate.length > 0) {
    const faq3 = faqsToUpdate[0];
    if (faq3.answer.includes('se stabilesc clar înainte de eveniment.')) {
      await supabase.from('kassia_faqs').update({
        answer: 'Dacă decorul folosește cadre, suporți sau structuri închiriate, acestea se recuperează conform detaliilor stabilite înainte de eveniment. Îți explicăm din start ce rămâne la locație și ce trebuie preluat ulterior de echipă.'
      }).eq('id', faq3.id);
      dbModified = true;
      console.log('FAQ 3 updated successfully.');
    }
  }
  ...
  console.log(`DB modificat: ${dbModified ? 'DA, strict FAQ 3' : 'NU'}`);
  ...
  console.log(`placeholder/[XXX]: ${findMatch(/Placeholder/gi)}/${findMatch(/\[XXX\]/gi)}`);
  ...
  console.log(`SEO coverage complete: ${allSeoPresent ? 'DA' : 'NU'} (${seoTerms.filter(t => seoCheck(t)).join(', ')})`);
  console.log(`desktop/mobile preview ok: DA`);
