-- SEED SKELETON PREVIEW PENTRU KASSIA V3 (169 PAGES)
-- GENERATED VIA SCRIPT.

BEGIN;

-- Ensure required columns exist for structural SEO and media slots
ALTER TABLE kassia_pages 
ADD COLUMN IF NOT EXISTS main_keyword VARCHAR(255),
ADD COLUMN IF NOT EXISTS search_intent VARCHAR(50);

ALTER TABLE kassia_gallery_items
ADD COLUMN IF NOT EXISTS slot_key VARCHAR(50),
ADD COLUMN IF NOT EXISTS label_owner VARCHAR(50),
...
-- Ensure unique constraints for idempotency on child tables
ALTER TABLE kassia_page_sections DROP CONSTRAINT IF EXISTS unique_page_section;
ALTER TABLE kassia_page_sections ADD CONSTRAINT unique_page_section UNIQUE(page_id, section_key);

ALTER TABLE kassia_faqs DROP CONSTRAINT IF EXISTS unique_page_faq;
ALTER TABLE kassia_faqs ADD CONSTRAINT unique_page_faq UNIQUE(page_id, sort_order);

ALTER TABLE kassia_gallery_items DROP CONSTRAINT IF EXISTS unique_page_slot;
