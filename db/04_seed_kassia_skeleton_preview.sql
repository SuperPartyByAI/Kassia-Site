-- SEED SKELETON PREVIEW PENTRU KASSIA (169 PAGES)
...
BEGIN
    ...
    INSERT INTO kassia_pages (slug, path, page_type, title, h1, meta_title, meta_description, canonical_url, priority, status, index_status, include_in_sitemap)
    VALUES ('arcada-baloane-nunta-bucuresti', '/arcada-baloane-nunta-bucuresti/', 'event', 'Arcada Baloane Nunta Bucuresti', 'Arcada Baloane Nunta Bucuresti', 'Arcada Baloane Nunta Bucuresti - Kassia', '', NULL, 0.5, 'draft', 'noindex', false)
    RETURNING id INTO v_page_id;
...
BEGIN
    ...
    INSERT INTO kassia_internal_links (source_page_id, target_page_id, anchor_text, reason)
    SELECT p1.id, p2.id, 'arcada baloane nunta bucuresti', 'portfolio_event'
    FROM kassia_pages p1, kassia_pages p2
    ...
    SELECT p1.id, p2.id, 'galerie evenimente', 'inspiration'
    FROM kassia_pages p1, kassia_pages p2
    WHERE p1.path = '/arcada-baloane-nunta-bucuresti/' AND p2.path = '/galerie/'
    ON CONFLICT (source_page_id, target_page_id, anchor_text) DO NOTHING;
